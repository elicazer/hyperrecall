"""``hyperrecall`` command-line interface.

Built on Typer. Subcommands::

    hyperrecall remember "text" --db mesh.db --participant Eli --topic TEDx
    hyperrecall recall  "query" --db mesh.db --budget 500
    hyperrecall export  mesh.db ./out
    hyperrecall import  ./out  mesh.db
...
def demo() -> None:
    ...
    result = mesh.recall("what is hyperrecall", budget_tokens=200)
    typer.echo(result.to_markdown())
    mesh.close()
