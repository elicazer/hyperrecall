"""Import a portable Markdown + YAML directory back into a HyperRecall database.

The inverse of :mod:`hyperrecall.portable.exporter`. Reads every ``nodes/*.md`` and
...
def import_dir(
    ...
    *,
    ...
    if not (root / "manifest.yaml").exists():
        raise FileNotFoundError(f"{root} is not a HyperRecall portable directory (no manifest.yaml)")
