...
_PASS2_SYSTEM = (
    ...
    "(resolve pronouns to names AND resolve relative time to a real date).\n"
    "- timestamp: ISO-8601 date (YYYY-MM-DD) of the EVENT the hyperedge "
    "describes. This is CRITICAL and often different from the utterance date:\n"
    "    * The turn header contains the utterance date in brackets, e.g. "
    "[Caroline at 2023-05-08] means Caroline said this on 2023-05-08.\n"
    "    * If the utterance says 'yesterday', event date = utterance_date - 1 day.\n"
    "    * 'last week' -> 7 days before. 'last month' -> the calendar month before.\n"
    "    * 'last year' -> utterance_year - 1. 'X years ago' -> subtract X years.\n"
    "    * 'today' or a currently-happening event -> utterance date.\n"
    "    * 'tomorrow' -> utterance_date + 1 day.\n"
    "    * An explicit date in the utterance -> use that date directly.\n"
    "    * If truly no time is implied or stated, timestamp may be null.\n"
    "    * ALWAYS resolve relative expressions; do NOT output timestamps like "
